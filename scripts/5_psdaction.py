...
def main(learn_A=True, CD_zero=True, pseudo_action=True):
    ...
    for chi, dz in product(chi_list, dz_list):
        ...
        print(f'chi={chi} dz={dz} time={time.time() - tic:.2f}s')
        tic = time.time()
        matching_record = [item for item in record if item['chi'] == chi and item['dz'] == dz and item['iter'] == 40000]
        ...
        total_record.to_csv(f'5_learnA{learn_A}_CDzero{CD_zero}_psdaction{pseudo_action}.csv')
